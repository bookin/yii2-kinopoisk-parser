...
class Kpparser {
    ...
    protected $main_url = 'https://www.kinopoisk.ru';
    ...
    public function getFilmData($id) {
        ...
        $parse = [
            ...
            'trailer_page' => '#a.*?href="(\/film\/\d*\/video\/\d+?\/)"#i'
        ];
        ...
        if($main_page){
            foreach($parse as $index => $value){
                if (preg_match($value, $main_page, $matches)) {
                    ...
                    } else if($index == 'trailer_page'){
                        $trailer_page = $this->getPage($this->main_url.$matches[1]);
                        if($trailer_page && preg_match($parse['trailer_url'], $trailer_page, $matches)){
                            if(preg_match('/getTrailersDomain[\s\S]*?\'(.*)\'/', $trailer_page, $domains)){
                                $trailer_url = sprintf('https://%s/%s', $domains[1], $matches[1]);
